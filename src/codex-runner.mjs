...
export const TELEGRAM_OUTPUT_INSTRUCTIONS = [
  ...
  'For the first 30 seconds after receiving a Telegram batch, use target decision=send with messageId=null; the connector mechanically removes reply quotes during that window.',
  'After 30 seconds, use target decision=reply with the exact messageId when the response directly answers or depends on one specific message; otherwise use send.',
...
export const TELEGRAM_BATCH_OUTPUT_INSTRUCTIONS = [
  ...
  'When using commentary within 30 seconds of receiving the batch, use a targeted send with the exact conversationKey and messageId=null. After 30 seconds, use a targeted reply when directly answering one specific message; otherwise use send. The bridge forwards at most the first commentary update to Telegram and keeps later commentary local.',
  ...
  'For the first 30 seconds after receiving the batch, use target decision=send with messageId=null; the connector mechanically removes reply quotes during that window.',
  'After 30 seconds, use target decision=reply with the exact Telegram message_id when directly answering or depending on one specific message. Otherwise use send. Do not reply merely because the input came from Telegram or because conversationKey is explicit.',
